...
const int m1 = 998244353, m2 = 1004535809, m3 = 469762049; int m;
const i64 m12 = (i64)m1 * m2;
const int _ = 3e5, inv1 = Pow(m1, m2 - 2, m2), inv2 = Pow(m12 % m3, m3 - 2, m3);
struct Int{
	int a, b, c;
...
int main(){
	read(N), read(M), read(m); int x;
	fp( i, 0, N ) read(x), a[i] = Int(x % m);
	fp( i, 0, M ) read(x), b[i] = Int(x % m);
	int n = Init(N + M + 1);
	NTT(a, n, 1), NTT(b, n, 1);
	fp( i, 0, n - 1 ) a[i] = a[i] * b[i];
	NTT(a, n, -1);
	fp( i, 0, N + M ) printf( "%d%c", a[i].get(), "\n "[i < n - 1] );
	return 0;
}