...
void NTT( int *a, int n, int opt ){
	...
	} if ( opt == -1 ) for ( int i = 0, t = Pow(n); i < n; ++i ) a[i] = (i64)a[i] * t % mod;
}

void work( int *a, int *b, int d ){
	if ( d == 1 ) return void(b[0] = Pow(a[0]));
	work(a, b, (d + 1) >> 1);
	int n(1), w(-1); while( n < (d << 1) ) n <<= 1, ++w;
	fp( i, 0, n - 1 ) r[i] = ( r[i >> 1] >> 1 ) | ( (i & 1) << w ), c[i] = i < d ? a[i] : 0;
	NTT(c, n, 1), NTT(b, n, 1); fp( i, 0, n - 1 ) b[i] = ( 2 + mod - (i64)b[i] * c[i] % mod ) * b[i] % mod; NTT(b, n, -1);
	fp( i, d, n - 1 ) b[i] = 0;
}
